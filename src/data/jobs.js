import labels from "./labels";

const data = [
  {
    default: true,
    start: "01/01/2024",
    end: "05/01/2024",
    title: "Full Stack Developper / Product Designer – Valence – ",
    description: `J'ai désigné l'UX / UI, développé et mis en production l'application web et le site
    vitrine de GodOfGames. C'est une application dans le domaine du jeu de plateau qui permet aux 
    utilisateurs de créer des événements sous forme d'olympiade - godofgames.io / godofgames.fr.`,
    tasks: [
      `Définition des règles métiers et du périmetre fonctionnel de l'application`,
      `Mise en place de la stack technique frontend, backend, devOps`,
      `Création du design et de l'expérience utilisateur`,
      `Déploiement et gestion de la solution en production`,
      `Développement de l'application, du back office et du site vitrine`,
      "Rencontre et animation des tests avec les utilisateurs",
    ],
    tools: [
      labels.typescript,
      labels.nextjs,
      labels.tailwind,
      labels.postgresql,
      labels.prisma,
      labels.clevercloud,
      labels.docker,
      labels.git,
      labels.vscode,
      labels.windows,
      labels.figma,
      labels.react,
      labels.git,
    ],
  },
  {
    default: true,
    start: "10/01/2021",
    end: "10/01/2023",
    title: "Full Stack Developer freelance à Linkeys – Grenoble – ",
    description: `Dans le cadre de la refonte de la plateforme de cooptation BtoB / BtoC app.linkeys.com, je suis intervenue sur de l'ensemble des solutions web et mobile.`,
    tasks: [
      "Développement d'un micro service interopérable pour intégrer les différentes solutions des systèmes de suivi des candidats du marché",
      "Mise en place d'un back office générique pour les professionnels pouvant être personnalisé en fonction des besoins des entreprises clientes",
      "Réalisation d'un back office spécifique pour analyser les données et optimiser le temps des équipes en interne",
      "Développement et optimisation de l'expérience utilisateurs et design graphique de l'ensemble des front office",
      "Participation au développement des applications mobiles à destination des stores.",
      "Correction de bugs et support de l'application en production.",
      "Formation de équipe support et rédaction documentation.",
    ],
    tools: [
      labels.javascript,
      labels.typescript,
      labels.primereact,
      labels.react,
      labels.redux,
      labels.node,
      labels.express,
      labels.postgresql,
      labels.typeorm,
      labels.docker,
      labels.codemagic,
      labels.aws,
      labels.vscode,
      labels.windows,
      labels.figma,
      labels.azure,
      labels.scrum,
    ],
  },
  {
    default: true,
    start: "12/01/2019",
    end: "09/01/2021",
    title: "Full Stack Developper / Product Designer à Mintset – Valence – ",
    description: `J'ai désigné l'UX / UI et participé au
    développement d'une application de résolution de planning par contraintes.`,
    tasks: [
      "Définition des règles métiers et du périmètre fonctionnel de l'application web",
      "Développement de la solution front et back du produit",
      "Mise en place d'un micro service de résolution de planning par contraintes",
      "Réalisation du design graphique et de l'expérience utilisateur de l'application web",
      "Rencontre avec les utilisateurs et analyse de leurs besoins",
      "Annimation des sessions de test avec les utilisateurs",
    ],
    tools: [
      labels.javascript,
      labels.react,
      labels.materialui,
      labels.redux,
      labels.node,
      labels.express,
      labels.mongodb,
      labels.docker,
      labels.vscode,
      labels.python,
      labels.windows,
      labels.figma,
    ],
  },
  {
    default: true,
    start: "07/01/2018",
    end: "12/01/2019",
    title: "Project Manager / Frontend Developper à Veryshop - Grenoble - ",
    description: `Au sein d'une équipe de 4 personnes, en tant que chef de projet et developpeur, j’ai coordonné et participé au 
    développement et la mise en production du produit VeryShop (site vitrine, application mobile, backoffice)`,
    tasks: [
      "Définition des règles métiers et du périmètre fonctionnel du produit",
      "Développement du back office et optimisation de la modération des utilisateurs",
      "Management de l'équipe et gestion agile",
      "Réunion et recettage avec le client",
      "Rédaction de spécifications et documentations.",

    ],
    tools: [
      labels.javascript,
      labels.react,
      labels.materialui,
      labels.redux,
      labels.vscode,
      labels.windows,
      labels.scrum,
    ],
  },
  // {
  //   start: "07/01/2017",
  //   end: "07/01/2018",
  //   title: "Software craftsmanship – Grenoble – ",
  //   description: `J'ai conçu, maquetté et développé une application Android dans le domaine de l'organisation. Sur cette période, je me suis formé sur le design graphique d'interface, l'expérience utilisateur et les problématiques business.`,
  //   tasks: [
  //     "Définition des règles métiers et du périmètre fonctionnel du produit",
  //     "Design et maquetage de l'application",
  //     "Développement de l'application Android",
  //     "Réalisation de l'état de l'art des différentes technologies",
  //     "Formation aux problématiques business et design graphique",
  //   ],
  //   tools: [
  //     labels.javaSE,
  //     labels.firebase,
  //     labels.SQLite,
  //     labels.windows,
  //     labels.androidStudio,
  //     labels.adobeXD,
  //     labels.git
  //   ],
  // },
  {
    start: "09/01/2013",
    end: "01/01/2017",
    title: "Ingénieur d'étude et développement à SOGETI High Tech – Montbonnot-Saint-Martin – ",
    description: `Dans le département R&D pour le compte d'Alstom Power puis General Electric renewable energy, j’ai conçu et développé un ensemble 
    d’applications Android interopérables dans le but de réduire le temps d’intervention dans les opérations de maintenance industrielle.`,
    tasks: [
      "Définition du périmètre fonctionnel des applications",
      "Développement de quatres applications Android interopérables",
      "POC d'accès aux applications avec des cartes magnétiques",
      "POC d'un accès aux bornes wifi avec des cartes magnetiques",
      "POC d'un dock pour tablette pour remplacer les écrans sur les armoires electrique",
    ],
    tools: [
      labels.javaSE,
      labels.androidStudio,
      labels.windows,
      labels.linux,
      labels.SQLite,
      labels.python,
      labels.SVN,
      labels.scrum
    ],
  },
  {
    start: "02/01/2013",
    end: "08/31/2013",
    title: "Ingénieur d'étude et développement à Alstom Power – Grenoble – ",
    description: `Dans le département R&D contrôle, j'ai réalisé la conception et le développement d'une application
    Android de réalitée augmentée (RA) dans le but de réduire le  temps d'intervertion dans les opérations de maintenance industrielle.`,
    tasks: [
      "Définition des règles métiers et du périmètre fonctionnel du produit",
      "Développement de l'application",
      "Réunion et recettage avec le client",
      "Rédaction de la documentation",
    ],
    tools: [
      labels.javaSE,
      labels.androidStudio,
      labels.SQLite,
      labels.windows,
      labels.SQLite,
      labels.SVN,

    ],
  },




  // {
  //   default: false,
  //   start: "09/01/2022",
  //   end: "01/01/2023",
  //   title: "Software craftsmanship à Mintset – Toulouse – ",
  //   description: `Dans le but d'explorer des nouveau outils et frameworks,
  //   j'ai developpé un CMS offrant une UX epuré et de grande
  //   performance. J'ai pu ensuite prospecter des clients et mettre
  //   en production différents sites de type vitrine ou blog.`,
  //   tasks: [
  //     "Création et mise en place d&apos;une nouvelle stack technique fullstack",
  //     "Réalisation du design du back-office",
  //     "Déploiement et gestion de la solution en production",
  //     "Animation du suivi de projet auprès des clients, définition du perimetre fonctionnel",
  //   ],
  //   tools: [
  //     labels.typescript,
  //     labels.nextjs,
  //     labels.tailwind,
  //     labels.mantine,
  //     labels.postgresql,
  //     labels.prisma,
  //     labels.gitlabci,
  //     labels.clevercloud,
  //     labels.git,
  //     labels.gitlab,
  //     labels.vscode,
  //     labels.linux,
  //   ],
  // },
  // {
  //   default: false,
  //   start: "08/01/2022",
  //   title: "Developpeur fullstack à La chouette Coop – Toulouse – ",
  //   description: `Au sein d'un supermarché coopératif, je participe à la gestion des outils informatiques et au developpement des nouveaux besoins.`,
  //   tasks: [
  //     "Prise en main et developpement des outils existant",
  //     "Mise en place des recommandantions de la CNIL au sujet de la RGPD",
  //     "Programmation d'un controlleur rfid pour commander la porte du magasin",
  //   ],
  //   tools: [
  //     labels.javascript,
  //     labels.typescript,
  //     labels.nextjs,
  //     labels.graphql,
  //     labels.appolo,
  //     labels.materialui,
  //     labels.php,
  //     labels.symphony,
  //     labels.mariadb,
  //     labels.netlify,
  //     labels.traefik,
  //     labels.rgpd,
  //     labels.git,
  //     labels.github,
  //     labels.vscode,
  //     labels.linux,
  //   ],
  // },
  // {
  //   default: true,
  //   start: "07/01/2021",
  //   end: "04/01/2022",
  //   title: "Architecte logiciel freelance à Linkeys – Toulouse – ",
  //   description: `Dans le cadre de la refonte du site app.linkeys.com afin d'améliorer l'expérience utilisateur et la maintenabilité du site.`,
  //   tasks: [
  //     "Participation aux choix d'architecture et leurs développements sur aws EC2 et cloudWatch",
  //     "Mise en place de la CI/CD sur azure devops et configuration des EC2 de test",
  //     "Mise en place de la stack technique frontend reactJS et Redux",
  //     "Mise en place de la stack technique backend expressJS, Typescript et typeORM",
  //     "Formation des développeurs junior sur la stack",
  //     "Mise en place de l'authentification jwt stocké dans un cookie et gestion du refreshToken",
  //     "Utilisation de la librairie primefaces React pour la UI",
  //     "Automatisation du build via codeMagic",
  //   ],
  //   tools: [
  //     labels.javascript,
  //     labels.typescript,
  //     labels.primereact,
  //     labels.react,
  //     labels.redux,
  //     labels.node,
  //     labels.express,
  //     labels.postgresql,
  //     labels.typeorm,
  //     labels.docker,
  //     labels.codemagic,
  //     labels.aws,
  //     labels.vscode,
  //     labels.linux,
  //     labels.mac,
  //   ],
  // },
  // {
  //   default: true,
  //   start: "09/01/2020",
  //   end: "04/01/2021",
  //   title: "Software craftsmanship à Mintset – Toulouse – ",
  //   description: `Création d'une application de résolution de contraintes de planning.`,
  //   tasks: [
  //     "Mise en place de l'architecture frontend, backend et infra",
  //     "Utilisation de la librairie Material UI pour l'interface",
  //     "Développement du backend en Python à l'aide de la librairie or-tools",
  //   ],
  //   tools: [
  //     labels.javascript,
  //     labels.materialui,
  //     labels.redux,
  //     labels.node,
  //     labels.express,
  //     labels.mongodb,
  //     labels.docker,
  //     labels.vscode,
  //     labels.linux,
  //   ],
  // },
  // {
  //   default: true,
  //   start: "10/01/2018",
  //   end: "10/01/2020",
  //   title: "Lead développeur à Parsys – Vincennes – ",
  //   description: `Maintien et amélioration d'une application client lourd nwjs embarqué pour des stations médicales de télémedecine. Developpement d'une application mobile en cordova pour l'ARS ile-de-france afin de permettre le suivi de patients entre différents professionnels de la santé.`,
  //   tasks: [
  //     "Correction de bugs et modernisation de la solution technique,",
  //     "Refonte de l'interface de visualisation de l'electrocardiogramme",
  //     "Animation de workshop technique et formation des autres developpeurs juniors",
  //     "Spécification, réalisation, et maintien de l'application IOS et Android",
  //     "Déploiement de l'application sur les stores Apple et google",
  //     "Coordination des developpements avec les autres prestataires du projet (Accenture et orion health)",
  //     "Fonctionnement en Agilité, sprint de 2 semaines",
  //   ],
  //   tools: [
  //     labels.javascript,
  //     labels.react,
  //     labels.redux,
  //     labels.materialui,
  //     labels.nw,
  //     labels.node,
  //     labels.cordova,
  //     labels.docker,
  //     labels.gitlab,
  //     labels.gitlabci,
  //     labels.scrum,
  //     labels.linux,
  //     labels.windows,
  //     labels.mac,
  //   ],
  // },
  // {
  //   default: true,
  //   start: "09/01/2016",
  //   end: "10/01/2018",
  //   title: "Développeur frontend à JobTeaser – Paris – ",
  //   description: `J'ai participé à la refonte de la plateforme afin de migrer le site vers une SPA en react.`,
  //   tasks: ["Onboarding", "ux"],
  //   tools: [
  //     labels.javascript,
  //     labels.react,
  //     labels.redux,
  //     labels.circleci,
  //     labels.docker,
  //     labels.github,
  //     labels.scrum,
  //     labels.linux,
  //   ],
  // },
  // {
  //   start: "10/01/2014",
  //   end: "05/01/2015",
  //   title: "Ingénieur de recherche à Middleware factory – Paris – ",
  //   description: `J'ai participé à un projet de recherche afin de créer un moteur de persistance orienté objet en java 8. J'ai notamment développé le module d’immutabilité des objets et mis en place les contraintes d’intégrité du système.`,
  //   tools: [labels.java],
  // },
  // {
  //   start: "06/01/2013",
  //   end: "09/01/2014",
  //   title: "Analyste développeur à CGI à CGI – Grenoble – ",
  //   description: `Au sein du département R&D, j'ai participé à la conception et à la réalisation d'une application de test AngularJs appliquée à un moteur générique de workflow en PL/SQL. J'ai aussi préparé et animé les comités de pilotage ainsi qu'assuré le support utilisateur en anglais.`,
  //   tools: [
  //     labels.javascript,
  //     labels.angular,
  //     labels.plsql,
  //     labels.scrum,
  //     labels.anglais,
  //   ],
  // },
];

export default data;
